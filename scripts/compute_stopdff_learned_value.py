...
BOOTSTRAP_REPS = 2000
PRODUCER_SCRIPT_PATH = "scripts/compute_stopdff_learned_value.py"
TRAINING_PRODUCER_SCRIPT_PATH = "scripts/train_stopdff_value_model.py"
...
def _committed_script_sha256(commit: str, script_path: str) -> str | None:
    injected_name = (
        "MODAL_HOST_TRAINER_SCRIPT_SHA256"
        if script_path == TRAINING_PRODUCER_SCRIPT_PATH
        else "MODAL_HOST_PRODUCER_SCRIPT_SHA256"
    )
...
def _require_exact_producer_binding(generation: dict[str, Any]) -> None:
...
def _validate_training_producer(
    ...
    *,
    ...
    if not isinstance(provenance, dict):
        raise ValueError(
            f"checkpoint {checkpoint_path} lacks training producer provenance"
        )
    script_path = provenance.get("script_path")
    if script_path != TRAINING_PRODUCER_SCRIPT_PATH:
        raise ValueError(
            f"checkpoint {checkpoint_path} has invalid canonical training producer path"
        )
    ...
    if (
        ...
        raise ValueError(f"checkpoint {checkpoint_path} has invalid producer SHA-256")
    ...
    if (
        ...
        raise ValueError(f"checkpoint {checkpoint_path} lacks a producer commit")
    if provenance.get("git_dirty"):
        raise ValueError(f"checkpoint {checkpoint_path} has a dirty training producer")
    if (
        provenance.get("commit_contains_exact_script") is not True
        or provenance.get("commit_script_sha256") != script_sha
    ):
        raise ValueError(
            f"checkpoint {checkpoint_path} does not bind its exact training producer"
        )
    current_commit = _current_git_commit()
    if current_commit is None:
        raise ValueError("cannot verify checkpoint producer without a source commit")
    if commit != current_commit:
        raise ValueError(
            f"checkpoint {checkpoint_path} was produced by a different source commit"
        )
    if payload.get("git_dirty") is not False:
        raise ValueError(
            f"checkpoint {checkpoint_path} has a dirty checkpoint save state"
        )
    if payload.get("git_sha") != commit:
        raise ValueError(
            f"checkpoint {checkpoint_path} checkpoint save commit does not match "
            "its validated training producer"
        )
    committed_sha = _committed_script_sha256(commit, script_path)
    if committed_sha != script_sha:
        raise ValueError(
            f"checkpoint {checkpoint_path} producer bytes do not match its source commit"
        )
    return commit
...
def _load_ensemble(
    ...
    *,
    ...
    checkpoint_producer_commits: set[str] = set()
    ...
    for path in checkpoint_paths:
        ...
        if before_sha != after_sha:
            raise RuntimeError(f"checkpoint changed while being loaded: {path}")
        checkpoint_sha256s[_display_path(path)] = before_sha
        checkpoint_producer_commits.add(
            _validate_training_producer(payload, checkpoint_path=path)
        )
    ...
    metadata = {
        ...
        "checkpoint_producer_commits": sorted(checkpoint_producer_commits),
...
def _generation_provenance(
    *,
    ...
    if calibration_path is not None:
        data_inputs.append(calibration_path)
    extras = [
        *data_inputs,
    ...
    generation["checkpoint_sha256s"] = dict(
        checkpoint_metadata.get("checkpoint_sha256s", {})
    )
    generation["input_sha256s"] = {
