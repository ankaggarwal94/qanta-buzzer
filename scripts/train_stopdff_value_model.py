...
def build_train_val_data(args: argparse.Namespace) -> tuple[TrajectoryBundle, TrajectoryBundle, dict[str, Any]]:
    ...
    calibration_path = (
        None if args.effective_identity_calibration else _resolve_calibration_path(args)
    )
    ...
    if calibration_path is not None:
        with calibration_path.open("r", encoding="utf-8") as f:
            calibration_meta = json.load(f).get("metadata", {})
        fit_split = str(calibration_meta.get("fit_split", "")).lower()
        if fit_split != str(args.train_split).lower():
            raise ValueError(
