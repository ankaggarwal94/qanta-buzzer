...
def test_stale_artifact_provenance_downgrades_overall_pass_to_warn() -> None:
    """A stale producer hash must affect the headline audit verdict.

    Warning only on stderr is too easy to miss: if all metric/data gates pass
    but a source artifact was generated by a stale producer script, the
    machine-readable card must not emit an unqualified overall PASS.
    """
    provenance = {
    ...
    overall, qualifier = make_audit_card._apply_artifact_provenance_to_overall(
        "PASS", None, provenance
    )

    assert overall == "WARN"
    assert qualifier == "stale producer hash for csli.json"


def test_stale_artifact_provenance_qualifies_existing_warn() -> None:
    """Existing WARNs should keep their reason and add stale-producer context."""
    provenance = {
        "csli.json": {"sha_matches": False},
        "calibration.json": {"sha_matches": False},
        "stopdff.json": {"sha_matches": True},
    }

    overall, qualifier = make_audit_card._apply_artifact_provenance_to_overall(
        "WARN", "retained-subset", provenance
    )

    assert overall == "WARN"
    assert qualifier == (
        "retained-subset; stale producer hash for csli.json, calibration.json"
    )
...
def test_modal_stage_shim_accepts_comma_and_whitespace_stage_lists() -> None:
    """Modal local_entrypoint hands ``stages`` to the shim as one string.

    The documented usage advertises whitespace-separated stages, while a
    comma-separated list is also convenient for one-token shell forwarding.
    Both forms must reconstruct argparse's ``nargs='+'`` shape.
    """
    import modal_cs321m

    assert modal_cs321m._split_modal_stages("build_mc_dataset compute_csli") == [
        "build_mc_dataset",
        "compute_csli",
    ]
    assert modal_cs321m._split_modal_stages("build_mc_dataset,compute_csli") == [
...
def test_compute_stopdff_report_generation_points_to_report_output() -> None:
    """stopdff_report.json must not reuse stopdff.json's output_path provenance."""
    repo_root = Path(__file__).resolve().parents[1]
    source = (repo_root / "scripts" / "compute_stopdff.py").read_text(
        encoding="utf-8"
    )

    assert "report_generation = build_generation_provenance(" in source
    assert "output_path=report_output_path" in source
    assert '"generation": report_generation' in source
