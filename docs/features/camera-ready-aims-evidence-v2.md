...
- **Phase-4 is not a sandbox or process boundary.** R-081 assumes a cooperative
  ...
  pre-acceptance byte binding. After promotion durability and
  producer-quarantine cleanup, the launcher durably publishes the negative
  `LAUNCH_ACCEPTANCE_PENDING.json` guard, fully publishes the canonical
  `LAUNCH_ACCEPTED.json` marker, and removes the guard as its terminal
  acceptance operation. The D7(b) driver rejects a surviving guard or missing
  positive marker even if a diagnostic STOP report could not be persisted.
